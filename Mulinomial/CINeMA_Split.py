    ...
    if args['tts_features']:
        ...
        if args['--w']:
            ...
            for i in range(len(set[:, 0])):
                indicies.append(i)
            X_train, X_test, y_train, y_test, indicies_train, indicies_test = train_test_split(set[:, 1:6].astype(float), set[:, 0], indicies, test_size=0.20, random_state=100)
            encoder = LabelEncoder()
            encoder.fit(y_train)
            y_train = encoder.transform(y_train)
        ...
        if args['--w']:
            ...
            print(len(X))
            print(len(X_test_2))
            model = rfi.model_train(X, Y, args['-s'])
