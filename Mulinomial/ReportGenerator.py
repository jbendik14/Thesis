...
class ReportGenerator:
    ...
    def report_csv(self, test, pred):
        ...
        i = 0
        for j in test:
            for (peak_num, name, confidence) in j:
    ...
    def report_pdf(self, hits, sample, valid_gt, pred, names):
        ...
        for v, s, h, n in zip(valid_gt, sample, hits, names):
            for (peak_num, name, confidence) in v:
                ...
                fig = plt.figure()
                t1,stemlines1,_t2 = plt.stem(sample_spectrum, markerfmt=" ", use_line_collection=True)
                plt.setp(stemlines1, linewidth=0.5, color='cornflowerblue')
                t1, stemlines, _t2 = plt.stem(-hit_spectrum, markerfmt=" ", use_line_collection=True)
        ...
        print('\n')
        print(high_correct_perc, high_correct, high_incorrect_perc, high_incorrect, low_correct_perc, low_correct, low_incorrect_perc, low_incorrect)
        pdf_correct.close()
        pdf_incorrect.close()
    ...
    def new_spectra(self, all_hits, all_samples, confidence):
        file_path = os.path.join(self.report_path, 'new_spectra_{}'.format(confidence))
        if not os.path.exists(file_path):
            os.mkdir(file_path)
        fig_path = os.path.join(file_path, "new_spectra.pdf")
        pdf = matplotlib.backends.backend_pdf.PdfPages(fig_path)
        for hit, sample in zip(all_hits, all_samples):
            hit_last_index = np.nonzero(hit)[0][-1]
            sample_last_index = np.nonzero(sample)[0][-1]
            max_index = max(hit_last_index, sample_last_index)
            hit = hit[0:max_index + 1]
            sample = sample[0:max_index + 1]
            hit[0:49] = 0
            hit_spectra = np.array(hit)
            sample_spectra = np.array(sample)
            fig = plt.figure()
            t1, stemlines1, _t2 = plt.stem(sample_spectra, markerfmt=" ", use_line_collection=True)
            plt.setp(stemlines1, linewidth=0.5, color='cornflowerblue')
            z1, stemlines, _z2 = plt.stem(-hit_spectra, markerfmt=" ", use_line_collection=True)
            plt.setp(stemlines, linewidth=0.5, color='coral')
            plt.xlabel('m/z')
            plt.ylabel('<---library/sample--->')
            pdf.savefig(fig)
    ...
    def original_spectra(self, hits, sample, valid_gt, pred, names):
        file_path = os.path.join(self.report_path, 'all_data')
        if not os.path.exists(file_path):
            os.mkdir(file_path)
        fig_path = os.path.join(file_path, "report.pdf")
